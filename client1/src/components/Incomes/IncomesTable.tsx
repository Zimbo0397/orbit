    ...
    <>
      ...
      <table>
        ...
        <tbody>
          ...
          {clientsPaginated.map((client: Income) => (
            <tr key={client.address}>
              <td className="border-b-0 lg:w-6 before:hidden">
                {/* <UserAvatar username={client.name} className="w-24 h-24 mx-auto lg:w-6 lg:h-6" /> */}
              </td>
              <td data-label="Тип">{client.amount}</td>
              <td data-label="Сума">{client.title}</td>
              <td data-label="Сума">{client.address}</td>
              <td data-label="Сума">{client.created_at}</td>
